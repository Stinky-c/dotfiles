...
def overview_button() -> widgets.Button:
    return widgets.Button(
        child=widgets.Icon(image="focus-windows-symbolic"),
        on_click=lambda _: niri.send_command({"Action":{"ToggleOverview":{}}}),
    )
...
"""
Idea:
Workspace dependent task bar icons - shows windows only in the current workspace
button in bottom left to activate overview
